# Temporal in Practice - Java: Instruqt Track

The seven exercises in this repo, packaged as an Instruqt lab. Attendees get a
container sandbox with a Temporal dev server already running, a warm Gradle
cache, the code editor on the exercise package, and the finished solution one
tab away.

```
instruqt/
├── track/       7 challenge directories + track.yml   ->  instruqt track push
...
| `sandbox/proxy/`, `src/main/java/...`, `src/test/java/...` | `git push` to `WORKSHOP_REF` | next sandbox provision |
...
The track does not exist remotely yet, so the first push creates it. `track.yml`
ships with a `slug:` and no `id:`, and the challenges and tabs have no `id:`
either. Instruqt assigns all of them on the first push; `instruqt track pull`
writes them back into the files, and they need committing so later pushes update
in place instead of creating duplicates.

Run from `instruqt/track/`:

```bash
cd instruqt/track
instruqt track validate    # local only, no push
instruqt track push        # first run creates the track
instruqt track pull        # writes the assigned ids back, then commit them
...
## What is different about the Java track

The Python track is the reference implementation for this layout. Five things
do not carry over, and all five are load-bearing.

**One Gradle module, fifteen packages.** `./gradlew classes` compiles every
exercise and every solution at once, so a syntax error in exercise 5 stops
exercise 1 from running. Every check script therefore has a compile gate that
runs before the Temporal-side gate, so the attendee is told about the compile
error rather than about a Workflow that never completed.

**Solve scripts rewrite packages, they do not copy files.** Every Java file
declares `package com.temporal.training.solutionN;`, so a plain `cp` into
`exerciseN` produces a package that does not match its directory. The solve
scripts `sed` the declaration on the way across. The file sets are identical
between each `exerciseN` and `solutionN`, which is what makes the loop safe.

**Every exercise has its own task queue.** Exercises 2 to 6 used to share
`MoneyTransferTaskQueue`. A Worker left running from a previous chapter then
picks up tasks for a Workflow it cannot replay, and the attendee gets a
non-determinism error pointing at code they never touched — or a Query that
fails with `Unknown query type: getStatus, knownTypes=[]`. The queues are
distinct now, and the setup and cleanup scripts still `pkill` stale Workers on
both entry and exit. The pkill patterns match the JVM command line
(`com.temporal.training.*StartWorker`), not a script name.

**Exercise 6 lives under `src/test`.** Its Code Editor and Solution tabs point
at `src/test/java/com/temporal/training/{exercise,solution}6`, and its check
runs `./gradlew test --rerun`. Without `--rerun` Gradle reports `UP-TO-DATE`
and the check passes on a stale result. The three `@Test` methods also ship
...
Running the starter is not enough to satisfy the check: the solve script has to
approve the transfer and then send the `retry` Signal with a valid account.
...
challenge: an upstream blip during warm-up (apt, Maven Central, Gradle's
distribution service, `temporal.download`) produces broken pooled sandboxes
silently.
